...
- 支持 Qwen3-VL 4B、8B 级别模型，并包含官方 FP8 预设，避免默认展示不适合普通本地显存的大模型。
- 支持 Qwen3.5 4B、9B、27B、35B-A3B 级别模型。
- 图片理解节点支持最多 6 张图片输入。
- 视频理解节点支持最多 3 路视频帧输入。
- 支持 OCR、视觉问答、图像描述、目标定位、目标检测、自定义提示词。
- 支持强制 JSON 提示，并提供 JSON 与坐标 JSON 辅助输出。
- 支持官方 FP8 模型预设，也支持 4bit、8bit 量化加载；量化是否可用取决于用户本地的 Python、CUDA、PyTorch 和 bitsandbytes 兼容情况。
...
- `Qwen3.5-Smit GGUF理解`
  ...
  使用 GGUF 量化模型和 llama.cpp 的 `llama-mtmd-cli` 进行文本或单图理解。这个节点不依赖 Transformers 对 `qwen3_5` 架构的支持，适合 Transformers 版本暂时不兼容、或者想使用 GGUF 量化模型的用户。GGUF 节点当前不做视频帧理解。
...
- GGUF 节点需要用户额外准备 llama.cpp 的 `llama-mtmd-cli` 可执行文件。只安装 `llama-cpp-python` 通常不会提供这个命令行文件。

### 显存建议

模型下拉栏默认只放入更适合本地 ComfyUI 的模型。Qwen3-VL 默认展示 4B、8B，以及对应的官方 FP8 预设。Qwen3.5 默认展示 4B、9B、27B、35B-A3B。更大的模型不会出现在默认列表中，避免普通用户误选后无法加载。
...
2. 本地模型后面会显示当前支持的模型 ID。
3. 如果选择模型 ID，并且本地没有该模型，节点会自动下载到对应的 ComfyUI 模型目录。
4. 自动下载顺序为：ModelScope 魔搭社区优先，Hugging Face 兜底。如果未安装 `modelscope`，会自动跳过 ModelScope。
...
本项目额外提供 `Qwen3.5-Smit GGUF理解` 节点。它参考了 [DanielBartolic/ComfyUI-Qwen3.5](https://github.com/DanielBartolic/ComfyUI-Qwen3.5) 的 GGUF 调用思路，通过 llama.cpp 的 `llama-mtmd-cli` 运行 Unsloth 提供的 Qwen3.5 GGUF 模型。

GGUF 模式和默认 Transformers 模式是两条独立路线：

- 默认 Qwen3.5 节点读取 Qwen 官方模型，需要 Transformers 支持 `qwen3_5` 架构。
- GGUF 节点读取 GGUF 文件，不依赖 Transformers 的模型架构支持。
- GGUF 节点需要 `llama-mtmd-cli`，并需要同时具备模型 GGUF 文件和 `mmproj-BF16.gguf`。
- 当前 GGUF 节点支持文本和单图理解，不支持视频帧理解。
...
如果没有把 `llama-mtmd-cli` 放进系统 PATH，可以在节点的 `llama路径` 输入里填写完整路径，例如：

```text
D:\llama.cpp\build\bin\Release\llama-mtmd-cli.exe
```

Windows CUDA 版 llama.cpp 通常需要用户自行按自己的显卡和 CUDA 环境编译。这个项目不会自动编译 llama.cpp。
...
- Qwen3-VL presets are limited to practical 4B and 8B options, including official FP8 presets.
...
- Supports official FP8 model presets, plus 4-bit and 8-bit loading when the user's Python, CUDA, PyTorch, and bitsandbytes stack is compatible.
...
- `Qwen3.5-Smit GGUF理解`
  ...
  Runs text or single-image understanding with GGUF models through llama.cpp `llama-mtmd-cli`. This path does not depend on Transformers support for the `qwen3_5` architecture. It currently does not support video-frame understanding.
...
- `modelscope` is optional. When installed, it is tried first for domestic downloads. If it is not compatible with the current Python version, the node automatically falls back to Hugging Face.
- The GGUF node requires a separate llama.cpp `llama-mtmd-cli` executable. Installing `llama-cpp-python` usually does not provide this CLI.

### VRAM Guide

The default model dropdown avoids very large models that are unrealistic for most local ComfyUI users. Qwen3-VL includes 4B, 8B, and matching official FP8 presets. Qwen3.5 includes 4B, 9B, 27B, and 35B-A3B presets.
...
2. Supported model IDs are listed after local models.
3. If a selected model ID is not available locally, it downloads into the matching ComfyUI model folder.
4. Automatic download order is ModelScope first, then Hugging Face. If `modelscope` is not installed, ModelScope is skipped automatically.
...
If you see `LocalEntryNotFoundError`, the node is trying to download a model but cannot reach ModelScope / Hugging Face, or the local cache does not contain a complete model.

Try these steps:

1. For domestic networks, install the optional ModelScope dependency first: `pip install modelscope`.
2. Check whether the machine can access ModelScope or Hugging Face.
3. Run `huggingface-cli login` if the selected model requires permission.
4. Set a Hugging Face endpoint before starting ComfyUI if your network needs one:

```bash
set HF_ENDPOINT=https://hf-mirror.com
```

5. To force a download source, set one of these before starting ComfyUI:
...
6. Or download the model manually and place the complete model folder under:
...
This project also provides `Qwen3.5-Smit GGUF理解`. It follows the GGUF approach used by [DanielBartolic/ComfyUI-Qwen3.5](https://github.com/DanielBartolic/ComfyUI-Qwen3.5): it runs Unsloth Qwen3.5 GGUF models through llama.cpp `llama-mtmd-cli`.

GGUF mode and Transformers mode are separate paths:

- The default Qwen3.5 nodes load official Qwen models and need Transformers support for `qwen3_5`.
- The GGUF node loads GGUF files and does not depend on Transformers model architecture support.
- The GGUF node needs both the model GGUF file and `mmproj-BF16.gguf`.
- The current GGUF node supports text and single-image understanding, not video-frame understanding.
...
If `llama-mtmd-cli` is not in PATH, set the full executable path in the node's `llama路径` input.
