...
Qwenvl-Smit 是一组面向本地 Qwen3-VL 工作流的 ComfyUI 自定义节点。它的目标是让用户可以直接在功能节点里选择模型，完成图片理解、多图参考、视频帧理解、OCR、视觉问答、目标定位、目标检测和 JSON 结构化输出。

本项目不包含任何模型权重。节点会优先读取 ComfyUI 的本地模型目录；如果选择的是支持的 Hugging Face 模型 ID，并且本地没有对应模型，节点会自动下载到 ComfyUI 的模型目录中。
...
- 支持 Qwen3-VL 4B、8B 级别模型，并包含官方 FP8 预设。
- 图片理解节点支持最多 6 张图片输入。
- 视频理解节点支持最多 3 路视频帧输入。
...
- `QwenVL-Smit 提示词预设`
  ...
  这个节点不会加载模型，也不会运行推理。它只负责生成可复用的提示词，适合把 OCR、图像描述、视觉问答、目标检测、目标定位、JSON 输出等常见任务预设给其他节点使用。
...
模型下拉栏默认只放入更适合本地 ComfyUI 的模型。Qwen3-VL 默认展示 4B、8B，以及对应的官方 FP8 预设。更大的模型不会出现在默认列表中，避免普通用户误选后无法加载。
...
4. 下拉栏只显示模型文件夹名或支持的模型 ID，不显示完整本机路径，也不会给重复模型添加括号后缀。
5. 如果选择模型 ID，并且本地没有该模型，节点会自动下载到对应的 ComfyUI 模型目录。
...
Qwen3-VL 模型由 Qwen 团队提供。本项目在这些模型的基础上做 ComfyUI 节点适配，包括中文界面、本地模型目录识别、多图输入、视频帧输入、JSON 辅助输出和适合本地显存的模型列表。使用模型时请遵守对应 Qwen 模型的开源协议与使用政策。

Qwenvl-Smit 节点代码本身使用 Apache-2.0 协议开源。

## English

Qwenvl-Smit is a ComfyUI custom node extension for local Qwen3-VL workflows. It lets users select models directly inside each functional node and run image understanding, multi-image reference, video-frame understanding, OCR, visual question answering, grounding, detection, and JSON-oriented structured output.

This project does not include model weights. It reads local models from ComfyUI model folders first. If a supported model ID is selected and the model is not available locally, the node downloads it into the matching ComfyUI model folder.
...
4. The dropdown shows only model folder names or supported model IDs, not full local paths and not duplicate suffixes.
