...
Qwenvl-Smit 是一组面向本地 Qwen3-VL 工作流的 ComfyUI 自定义节点。它的目标是让用户可以直接在功能节点里选择模型，完成智能问答、图片理解、多图参考、视频帧理解、OCR、视觉问答、目标定位、目标检测和 JSON 结构化输出。
...
Qwenvl-Smit is a ComfyUI custom node extension for local Qwen3-VL workflows. It lets users select models directly inside each functional node and run smart Q&A, image understanding, multi-image reference, video-frame understanding, OCR, visual question answering, grounding, detection, and JSON-oriented structured output.
